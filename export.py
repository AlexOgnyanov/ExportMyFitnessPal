...
while(date_to_get != -1):
  ...
  if not date.totals:
    ...
    print(f"Written - - - - {config['1']} for {date_to_get[0]}.{date_to_get[1]}.{date_to_get[2]}")
  else:
    write_data_to_sheet([date.totals['calories'], date.totals['protein'], date.totals['carbohydrates'], date.totals['fat'], config["5"]], working_sheet)
